...
async function getLeadPayload(slug) {
  ...
  return {
    ...
    contact_id: contact.id,
    campaign_id: contact.campaign_id || '',
    screenshot_windows: DEFAULT_SCREENSHOT_WINDOWS
  };
}
...
app.get('/w/:slug', async (req, res) => {
  ...
  if (LANDING_PAGE_DOMAIN) {
    return res.redirect(302, `${LANDING_PAGE_DOMAIN}/w/${slug}`);
  }

  const payload = await getLeadPayload(slug);
  if (!payload) return res.status(404).send('This link is no longer active.');

  const template = fs.readFileSync(path.join(__dirname, 'landing-page-template.html'), 'utf8');
  const html = template
    .replace(/\{\{business_name\}\}/g, payload.business_name)
    .replace(/\{\{video_url\}\}/g, payload.video_url)
    .replace(/\{\{screenshot_url\}\}/g, payload.screenshot_url)
    .replace(/\{\{booking_url\}\}/g, payload.booking_url)
    .replace(/\{\{assigned_inbox\}\}/g, payload.assigned_inbox)
    .replace(/\{\{original_subject\}\}/g, encodeURIComponent(payload.original_subject))
    .replace(/\{\{contact_id\}\}/g, payload.contact_id)
    .replace(/\{\{campaign_id\}\}/g, payload.campaign_id)
    .replace(/\{\{screenshot_windows\}\}/g, JSON.stringify(payload.screenshot_windows));
