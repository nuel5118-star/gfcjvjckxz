...
const APP_URL = (process.env.APP_URL || 'https://gfcjvjckxz.onrender.com').replace(/\/$/,'');

// Custom domain now hosting the personalized landing page (landing-page-template.html)
// as a static file, separate from this Railway app. Can be overridden via the
// LANDING_PAGE_DOMAIN env var in Railway if this ever changes.
const LANDING_PAGE_DOMAIN = (process.env.LANDING_PAGE_DOMAIN || 'https://www.botcipherhq.site').replace(/\/$/,'');
...
function applyVariables(template,contact,customFields){
  if(!template)return '';
  // contact is spread last so real column values win over computed
  // customFields (e.g. a contact's own gif_url beats the niche-wide
  // fallback) -- but a NULL/undefined column (a contact that just hasn't
  // been backfilled yet) would otherwise silently wipe out that fallback
  // too, since null still overrides in a spread. Strip null/undefined
  // columns first so only real values from contact can override.
  const contactValues=Object.fromEntries(Object.entries(contact||{}).filter(([,v])=>v!==null&&v!==undefined));
  const data={first_name:'',last_name:'',company:'',city:'',phone:'',business_url:'',timezone:'',...(customFields||{}),...contactValues};

  // {{#if_name}}...{{/if_name}} -- the whole block is dropped entirely
  // when first_name is blank, instead of falling back to a generic
  // "there". Use this to make a full greeting LINE disappear for
  // nameless leads rather than showing "Hey there,". Resolved before the
  // main {{var}} pass below, so normal tags still work inside the block.
  const hasName=!!String(data.first_name||'').trim();
  template=template.replace(/\{\{#if_name\}\}([\s\S]*?)\{\{\/if_name\}\}/g,(match,inner)=>hasName?inner:'');
...
async function getSettings(){
  // Always the SAME row on every call, oldest first -- without an
  // explicit order, .limit(1) on more than one row is nondeterministic,
  // which is what let GET and PUT disagree on which row was "the"
  // settings row (a save could succeed on one row while the page kept
  // reading another).
  const{data,error}=await supabase.from('settings').select('*').order('created_at',{ascending:true}).limit(1);
...
// FIX: allowed_niches was displayed and editable in the Inboxes UI but was
// never actually enforced anywhere — POST/PUT /api/inboxes silently dropped
// the field (never saved to the DB) and the scheduler's inbox round-robin
// never looked at it at all. Both are fixed below; this is the shared filter
// both call sites now use. An inbox with no allowed_niches (empty/null) is
// unrestricted and can send to any niche.
function inboxesForNiche(inboxes,niche){
  return (inboxes||[]).filter(i=>!i.allowed_niches?.length||i.allowed_niches.includes(niche));
}
...
// ── SEQUENCE STEP TOGGLING ──────────────────────────────────────────────────
// Resolves the actual step a contact should be on, given their current_step
// pointer. If the step at current_step is disabled (or was removed), this
// walks forward to the next ENABLED step at or after that number. This lets
// a user toggle a step off mid-campaign — contacts sitting on it (or arriving
// at it) transparently skip to the next enabled one instead of getting stuck
// or receiving disabled content.
function resolveStep(steps,fromStepNumber){
  return (steps||[])
    .filter(s=>s.enabled!==false&&s.step_number>=fromStepNumber)
...
function injectTracking(body, params){
  ...
  // FIX: this used to flip to "treat the whole body as HTML, skip \n->
  // conversion entirely" the instant ANY tag appeared anywhere in the body —
  // including the single inline <a><img/></a> snippet the Insert GIF/Video
  // button drops in. That meant every line break in the email (not just near
  // the image) collapsed to nothing once a mail client rendered it, so the
  // sign-off ended up glued onto the last line of body text. Now only actual
  // BLOCK-level HTML (someone pasted a real template with their own <p>/
  // <div>/<br>/<table>/etc.) is treated as "author handled their own line
  // breaks" — a plain-text body with one inline snippet still gets its own
  // newlines converted to <br>, same as pure plain text always has.
  const hasBlockHtml=/<\/?(p|div|table|tr|td|th|br|ul|ol|li|h[1-6]|body|html)\b/i.test(body);
  const hasAnyHtml=/<[a-z][\s\S]*>/i.test(body);
  if(!hasBlockHtml){
    if(!hasAnyHtml){
      // Pure plain text — safe to escape stray <, >, & before adding <br>s
      body=body.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;');
    }
    // Escaping is skipped when an inline snippet is present so its real
    // <a>/<img> tags aren't corrupted — but the line-break conversion still
    // runs either way, since the user's own newlines are still meaningful.
    body=body
...
// The video has a fixed sequence of on-screen "stages" — each niche's video
// is a different length so the stage cutoffs differ per niche:
//   stage 1 (0 → stage1_end):        personalized website screenshot
//   stage 2 (stage1_end → stage2_end): shared "how it works" image
//   stage 3 (stage2_end → stage3_end): shared "system" image
//   stage 4 (stage3_end → end):       no overlay — presenter goes full-screen
// Add video_stage{1,2,3}_end_<niche> columns to settings in Supabase to set
// these per niche (seconds, as numbers).
function getVideoTimelineForNiche(settings, niche){
  const suffix = ['lawn_care','irrigation','tree_removal'].includes(niche) ? niche : 'tree_removal';
  return {
    stage1_end: Number(settings[`video_stage1_end_${suffix}`]) || 0,
    stage2_end: Number(settings[`video_stage2_end_${suffix}`]) || 0,
    stage3_end: Number(settings[`video_stage3_end_${suffix}`]) || 0,
  };
}
...
// GIF preview shown INSIDE the email itself (the Loom-style "thumbnail that
// looks like a playing video" trick) — clicking it is what takes the lead to
// the actual landing page. Same per-niche pattern as the video, just add
// gif_url_lawn_care / gif_url_irrigation / gif_url_tree_removal columns to
// the settings row in Supabase once each GIF is ready.
function getGifUrlForNiche(settings, niche){
  const map = {
    lawn_care: settings.gif_url_lawn_care,
    irrigation: settings.gif_url_irrigation,
    tree_removal: settings.gif_url_tree_removal,
  };
  return map[niche] || map.tree_removal || '';
}

// The personalized landing page link that goes IN the email (wrapped around
// the GIF, or as a plain {{watch_link}} merge tag). Points at the custom
// domain once LANDING_PAGE_DOMAIN is set, same as the /w/:slug redirect.
function buildWatchLink(slug){
  if (!slug) return '';
  return `${LANDING_PAGE_DOMAIN || APP_URL}/w/${slug}`;
}

// Shared lookup used by both the legacy server-rendered /w/:slug route and
// the JSON /api/lead/:slug route the static landing page (on the custom
// domain) now fetches from client-side. Keeping one function means the two
// routes can never quietly drift out of sync on what fields/logic they use.
async function getLeadPayload(slug) {
  ...
  const timeline = getVideoTimelineForNiche(settings, contact.niche);
  ...
  return {
    ...
    how_it_works_url: settings.how_it_works_image_url || '',
    system_url: settings.system_image_url || '',
    video_stage1_end: timeline.stage1_end,
    video_stage2_end: timeline.stage2_end,
    video_stage3_end: timeline.stage3_end,
...
// ── LANDING PAGE JSON API ─────────────────────────────────────────
// The static landing page now lives on the custom domain, not on Railway,
// so it can't be server-templated anymore — it fetches this instead, on
// load, using the slug from its own URL. CORS is already open (app.use(cors())
// above) so the custom domain can call this cross-origin.
app.get('/api/lead/:slug', async (req, res) => {
  const payload = await getLeadPayload(req.params.slug);
  if (!payload) return res.status(404).json({ error: 'This link is no longer active.' });
  res.json(payload);
});

// ── VIDEO WATCH TRACKING ─────────────────────────────────────────
// Called from the landing page JS on 'play' (first play only) and on
// 'ended'. Logged as its own email_events row (so it shows up in Analytics
// same as opens/clicks) AND fed into the same branch-trigger mechanism as
// every other trigger_type, so intent tracks can react to "watched the
// video" / "finished the video" exactly like they react to a link click.
app.post('/api/track/video', async (req, res) => {
  const { contact_id, campaign_id, event } = req.body || {};
  if (!contact_id || !event) return res.status(400).json({ error: 'contact_id and event are required' });
  const triggerType = event === 'complete' ? 'video_complete' : 'video_play';
  try {
    await supabase.from('email_events').insert({
      type: triggerType,
      contact_id,
    ...
    if (campaign_id) {
      checkBranchTrigger({ contact_id, campaign_id, trigger_type: triggerType, trigger_meta: {} }).catch(() => {});
    }
    res.json({ ok: true });
  } catch (e) {
    console.error('[track/video]', e.message);
    res.status(500).json({ error: e.message });
  }
});

// ── GENERIC LANDING-PAGE EVENT TRACKING ────────────────────────────
// Same shape as /api/track/video, generalized for the newer landing-page
// interactions (calculator, FAQ, booking clicks) that don't have their own
// route yet. Logged as its own email_events row AND fed into
// checkBranchTrigger, same as every other trigger_type, so intent tracks
// can react to these exactly like a link click or a calculator submit.
// Extra context that doesn't have its own column on email_events (which
// FAQ question, watch seconds, etc.) goes into `subject` as a short
// human-readable label -- same trick /track/calculator already uses for
// clicked_url/subject -- so this needs no schema change.
function describeTrackEvent(event, meta) {
  meta = meta || {};
  switch (event) {
    case 'video_progress': return `Video ${meta.percent || 0}%`;
    case 'video_watch_time': return `Watched ${meta.seconds || 0}s`;
    case 'calc_interacted': return 'Calculator interacted';
    case 'calc_revealed': return `Calculator revealed ($${Math.round(meta.total || 0).toLocaleString('en-US')})`;
    case 'faq_opened': return `FAQ: ${meta.question || 'unknown'}`;
    case 'booking_clicked': return `Booking click: ${meta.section || 'unknown'}`;
    default: return event;
  }
}
app.post('/api/track/event', async (req, res) => {
  const { contact_id, campaign_id, event, meta } = req.body || {};
  if (!contact_id || !event) return res.status(400).json({ error: 'contact_id and event are required' });
  try {
    await supabase.from('email_events').insert({
      type: event,
      contact_id,
      campaign_id: campaign_id || null,
      subject: describeTrackEvent(event, meta),
      created_at: new Date().toISOString()
    });
    if (campaign_id) {
      checkBranchTrigger({ contact_id, campaign_id, trigger_type: event, trigger_meta: meta || {} }).catch(() => {});
    }
    res.json({ ok: true });
  } catch (e) {
...
// ── LANDING PAGE ROUTE (legacy Railway redirect) ──────────────────
// Any /w/:slug hit on Render 302s straight to the custom domain — so any
// link already built as APP_URL + /w/<slug> anywhere in the app (or
// already sent in a past email) keeps working, it just bounces to the
// real page. The page itself is 100% client-rendered now (fetches
// /api/lead/:slug), so there's no server-side template to fall back to.
app.get('/w/:slug', async (req, res) => {
  res.redirect(302, `${LANDING_PAGE_DOMAIN || APP_URL}/w/${req.params.slug}`);
});
...
app.post('/api/campaigns/:id/contacts/import',async(req,res)=>{
  ...
  for(const record of records){
    ...
    const{data:inserted,error}=await supabase.from('contacts').upsert({campaign_id:campaignId,email,first_name:formatName(contact.first_name||''),last_name:formatName(contact.last_name||''),company:contact.company||'',city:contact.city||'',phone:contact.phone||'',business_url:contact.business_url||'',timezone:contact.timezone||'',niche:contact.niche||'',screenshot_url:contact.screenshot_url||'',gif_url:contact.gif_url||'',custom_fields:customFields,status:'active',current_step:0,enrolled_at:new Date().toISOString()},{onConflict:'campaign_id,email'}).select().single();
...
app.delete('/api/campaigns/:id',async(req,res)=>{
  ...
  // Previously this only cleared contacts + campaign_steps, then deleted
  // the campaign and always responded {ok:true} -- with no error checks
  // at all. campaign_branches (attached intent-track rules), email_events,
  // and sequence_sends also reference campaign_id; if a campaign has ever
  // had a send or an attached track, those rows block the final delete via
  // foreign key, and the old code had no way to know that happened -- it
  // reported success regardless. Now every referencing table is cleared
  // first, and a real failure is actually surfaced instead of hidden.
  const steps=[
    ()=>supabase.from('contacts').delete().eq('campaign_id',id),
    ()=>supabase.from('campaign_steps').delete().eq('campaign_id',id),
    ()=>supabase.from('campaign_branches').delete().eq('campaign_id',id),
    ()=>supabase.from('email_events').delete().eq('campaign_id',id),
    ()=>supabase.from('sequence_sends').delete().eq('campaign_id',id),
    ()=>supabase.from('campaigns').delete().eq('id',id),
  ];
  for(const step of steps){
    const{error}=await step();
    if(error){
      console.error(`[Delete Campaign ${id}] Failed:`,error.message);
...
// ── SEQUENCES LIBRARY ────────────────────────────────────────────────────────
// A "sequence" is a reusable, standalone set of emails (like a template),
// independent of any one campaign. When building a campaign you can pull
// individual emails from any sequence into any slot — see /api/campaigns
// steps[].source_sequence_step_id. Editing a sequence later does NOT change
// campaigns that already copied from it (copy, not live-link), so a running
// campaign never changes underneath you.
app.get('/api/sequences',async(req,res)=>{const{data,error}=await supabase.from('sequences').select('*, sequence_steps(*)').order('created_at',{ascending:false});if(error)return res.status(500).json({error:error.message});const sorted=(data||[]).map(s=>({...s,sequence_steps:(s.sequence_steps||[]).sort((a,b)=>a.step_number-b.step_number)}));res.json(sorted);});
app.get('/api/sequences/:id',async(req,res)=>{const{data,error}=await supabase.from('sequences').select('*, sequence_steps(*)').eq('id',req.params.id).single();if(error)return res.status(404).json({error:'Not found'});data.sequence_steps=(data.sequence_steps||[]).sort((a,b)=>a.step_number-b.step_number);res.json(data);});
...
app.put('/api/sequences/:id',async(req,res)=>{
  const{name,steps}=req.body;
  const{data:sequence,error}=await supabase.from('sequences').update({name}).eq('id',req.params.id).select().single();
  if(error)return res.status(500).json({error:error.message});
  if(steps){
...
app.post('/api/campaigns/:id/launch',async(req,res)=>{
  ...
  // FIX: inbox assignment was a single global round-robin (`inboxes[i%inboxes.length]`)
  // that completely ignored each inbox's allowed_niches restriction — a lawn-care-only
  // inbox could easily end up sending to a tree-removal contact. Round-robin index is
  // now tracked PER niche-eligible-inbox-pool, so contacts of the same niche still
  // spread evenly across just the inboxes allowed to send to them.
  const niboxIdx={};
  let unassignable=0;
  for(let i=0;i<contacts.length;i++){
    if(countOnCurrentDay>=maxPerDay){dayOffset++;countOnCurrentDay=0;}
    const contact=contacts[i];
    const eligibleInboxes=inboxesForNiche(inboxes,contact.niche);
    if(!eligibleInboxes.length){
      // No inbox is allowed to send to this contact's niche — skip rather than
      // silently violating the restriction by assigning one anyway. Contact stays
      // status='active', current_step=0 (never launched) so it's easy to spot and
      // re-run once an eligible inbox exists.
      unassignable++;
      continue;
    }
    const poolKey=contact.niche||'__any__';
    const idx=niboxIdx[poolKey]||0;
    const inbox=eligibleInboxes[idx%eligibleInboxes.length];
    niboxIdx[poolKey]=idx+1;
  ...
  await supabase.from('campaigns').update({status:'active',updated_at:new Date().toISOString()}).eq('id',req.params.id);
  res.json({ok:true,scheduled:contacts.length-unassignable,unassignable});
});
...
app.post('/api/preview',async(req,res)=>{
  ...
  const c=contact||{first_name:'John',last_name:'Smith',company:'Acme Corp',city:'Lagos',phone:'080-1234-5678',business_url:'acmecorp.com',timezone:'Africa/Lagos',watch_link:`${LANDING_PAGE_DOMAIN||APP_URL}/w/preview123`,gif_url:'https://example.com/preview.gif'};
...
// FIX: allowed_niches was missing from both the insert and update payloads below —
// the Inboxes page sent it, the DB column may have held it, but it was silently
// discarded on every save. Now persisted, defaulting to [] (unrestricted) on create.
app.post('/api/inboxes',async(req,res)=>{const{email,label,daily_cap,allowed_niches}=req.body;if(!email||!isValidEmail(email))return res.status(400).json({error:'Invalid email'});const{data,error}=await supabase.from('inboxes').upsert({email:normalizeEmail(email),label,daily_cap:daily_cap||100,allowed_niches:allowed_niches||[],active:true},{onConflict:'email'}).select().single();if(error)return res.status(500).json({error:error.message});res.json(data);});
app.put('/api/inboxes/:id',async(req,res)=>{const{label,active,daily_cap,allowed_niches}=req.body;const{data,error}=await supabase.from('inboxes').update({label,active,daily_cap,allowed_niches}).eq('id',req.params.id).select().single();if(error)return res.status(500).json({error:error.message});res.json(data);});
app.delete('/api/inboxes/:id',async(req,res)=>{await supabase.from('inboxes').delete().eq('id',req.params.id);res.json({ok:true});});

// FIX: the Inboxes page calls this to populate the niche-restriction chips
// but the route never existed — every call 404'd (silently swallowed by the
// frontend's .catch(()=>{})), so the niche list was always empty and nobody
// could ever actually select a niche to restrict an inbox to. Derived from
// whatever niche values are actually present on contacts (set at CSV import).
app.get('/api/niches',async(req,res)=>{
  const rows=await fetchAll(()=>supabase.from('contacts').select('niche').not('niche','is',null).neq('niche',''));
  const niches=[...new Set(rows.map(r=>r.niche).filter(Boolean))].sort();
  res.json(niches);
});
...
app.put('/api/settings',async(req,res)=>{
  ...
  let result,dbError;
  if(existing.id){
    const{data,error}=await supabase.from('settings').update({...req.body,updated_at:new Date().toISOString()}).eq('id',existing.id).select().single();
    result=data;dbError=error;
  }else{
    const{data,error}=await supabase.from('settings').insert(req.body).select().single();
    result=data;dbError=error;
  }
  // Previously this ignored `error` entirely and always responded 200 with
  // whatever `data` was (including null) -- a silently-failed update (RLS,
  // bad column, etc.) looked identical to a successful save on the
  // frontend, which only checked whether the request threw. Now a DB
  // failure actually returns an error status so "Save Settings" stops
  // claiming success when nothing was written.
  if(dbError){console.error('[Settings] Save failed:',dbError.message);return res.status(500).json({error:dbError.message});}
  res.json(result);
});
...
async function runForceSendTest(campaignId){
  ...
  const contacts=await fetchAll(()=>supabase.from('contacts')
    .select('id,email,first_name,last_name,company,city,phone,business_url,gif_url,custom_fields,current_step,assigned_inbox,campaign_id,track,page_slug,niche')
    .eq('status','active')
  ...
  const niboxIdx={};// FIX: same niche-aware round-robin as /launch — see inboxesForNiche
  ...
  for(const contact of contacts){
    ...
    if(!inbox){
      // FIX: was a plain global round-robin over ALL inboxes, ignoring allowed_niches
      const eligibleInboxes=inboxesForNiche(inboxes,contact.niche);
      if(!eligibleInboxes.length){result.skipped++;result.skip_reasons.no_inbox_available=(result.skip_reasons.no_inbox_available||0)+1;continue;}
      const poolKey=contact.niche||'__any__';
      const idx=niboxIdx[poolKey]||0;
      inbox=eligibleInboxes[idx%eligibleInboxes.length].email;
      niboxIdx[poolKey]=idx+1;
    }

    const customFields={
      ...(contact.custom_fields||{}),
      watch_link: buildWatchLink(contact.page_slug),
      // Per-lead gif_url (from the leadvideo pipeline) takes priority --
      // this was previously ALWAYS using the shared per-niche settings
      // value instead, which meant every contact's personalized gif_url
      // column was silently ignored at send time. The niche-level setting
      // is now only a fallback for a contact that somehow has none.
      //
      // The email now points at our own domain's proxy route instead of
      // the raw Supabase storage URL -- mobile mail clients / spam filters
      // were flagging the random Supabase project hostname, causing the
      // GIF to fail to load on mobile while working fine on desktop.
      // The Vercel route (/api/gif/<contact id>) fetches the real file
      // from Supabase server-side and serves it back under our own domain.
      gif_url: (contact.gif_url || getGifUrlForNiche(settings, contact.niche))
        ? `https://botcipherco.site/api/gif/${contact.id}`
        : ''
    };
    ...
    emailBatch.push({to:contact.email,subject,body:trackedBody,inbox,campaign_id:campaign.id,campaign_name:campaign.name,contact_id:contact.id,step:step.step_number,send_id:sendId,first_name:contact.first_name,last_name:contact.last_name,company:contact.company});
    batchMeta.push({sendId,campaign_id:campaign.id,contact_id:contact.id,email:contact.email,inbox,step_number:step.step_number,subject,body:rawBody,campaign_name:campaign.name,advancePayload});
  }
    ...
    while(true){
      ...
      for(const campaign of activeCampaigns){
        ...
        const{data:batch,error:queryError}=await supabase.from('contacts')
          .select('id,email,first_name,last_name,company,city,phone,business_url,gif_url,custom_fields,current_step,next_send_at,assigned_inbox,campaign_id,track,page_slug,niche')
          .eq('status','active')
          .eq('campaign_id',campaign.id)
        ...
        for(const contact of batch){
          ...
          const customFields={
            ...(contact.custom_fields||{}),
            watch_link: buildWatchLink(contact.page_slug),
            // Same fix as the other send path above -- per-contact gif_url
            // wins, niche-level setting is only a fallback.
            //
            // Points at our own domain's proxy route instead of the raw
            // Supabase storage URL -- see comment in the other send path
            // above for why (mobile mail clients flagging the Supabase
            // hostname, breaking the GIF on mobile only).
            gif_url: (contact.gif_url || getGifUrlForNiche(settings, contact.niche))
              ? `https://botcipherco.site/api/gif/${contact.id}`
              : ''
          };
...
app.post('/api/scheduler/retry/:sendId',async(req,res)=>{
  ...
  const{data:campaign}=await supabase.from('campaigns').select('id,name,send_hour_start,send_hour_end,skip_weekends,timezone,random_delay_max,campaign_steps(*)').eq('id',send.campaign_id).single();
  try{
    // FIX: this was posting send.body (the RAW stored body) straight to the
    // webhook, completely bypassing injectTracking — every retried email went
    // out with no tracking pixel, no click-tracking, no unsubscribe footer,
    // and (before the newline fix above) collapsed line breaks too. Now built
    // through the exact same injectTracking() call every normal send uses.
    const links=await getLinks();
    const trackedBody=injectTracking(send.body,{email:send.email,inbox:send.inbox,campaign_id:send.campaign_id,campaign_name:campaign?.name||'',contact_id:send.contact_id,step:send.step_number,send_id:send.id,subject:send.subject,links});
    ...
    await axios.post(settings.webhook_url,{
      batch:[{to:send.email,subject:send.subject,body:trackedBody,inbox:send.inbox,
    ...
    if(campaign){
      const steps=(campaign.campaign_steps||[]).sort((a,b)=>a.step_number-b.step_number);
      const nextStep=resolveStep(steps,send.step_number+1);// skips disabled steps forward
      const now=new Date();
      if(nextStep){
        // Use next step's own hours if set, fall back to campaign hours
