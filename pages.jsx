...
// ── DAILY TREND CHART — hand-rolled SVG line chart, no chart library needed ──
function DailyTrendChart({ daily }) {
  ...
  const series = [
    { key:'delivered', color:'#057a55', label:'Delivered' },
    { key:'opens',     color:'#1a56db', label:'Opens' },
    { key:'clicks',    color:'#7e3af2', label:'Clicks' },
    { key:'replies',   color:'#8e4b10', label:'Replies' },
  ];
  ...
  // Show up to ~6 evenly-spaced date labels along the x-axis
    ...
    <div>
      <svg viewBox={`0 0 ${W} ${H}`} style={{ width:'100%', height:'auto', display:'block' }}>
        ...
        {[0, 0.25, 0.5, 0.75, 1].map(f => (
          <line key={f} x1={PAD_L} x2={W - PAD_R} y1={PAD_T + plotH * (1 - f)} y2={PAD_T + plotH * (1 - f)}
            stroke="var(--border)" strokeWidth="1" />
        ))}
        {/* Y-axis labels */}
        {[0, 0.5, 1].map(f => (
          <text key={f} x={4} y={PAD_T + plotH * (1 - f) + 4} fontSize="10" fill="var(--text-muted)">
            {Math.round(maxV * f)}
          </text>
        ))}
        {/* X-axis date labels */}
        {data.map((d, i) => (i % labelEvery === 0 || i === data.length - 1) ? (
          <text key={d.date} x={x(i)} y={H - 6} fontSize="9" fill="var(--text-muted)" textAnchor="middle">
            {d.date.slice(5)}
          </text>
        ) : null)}
        {/* Series lines */}
        ...
        {/* Dots on the most recent point per series */}
        {series.map(s => (
          <circle key={s.key} cx={x(data.length - 1)} cy={y(data[data.length - 1][s.key] || 0)} r="3" fill={s.color} />
        ))}
      </svg>
...
// ── ANALYTICS — summary metrics, trend, and campaign breakdown ───────────────
export function AnalyticsPage() {
  ...
  useEffect(() => {
    ...
    api.getAnalytics(p).then(setStats).finally(() => setStatsLoading(false));
  }, [date, campaignId]);

  const t = stats?.totals || {};
  const r = stats?.rates || {};

  // Delivered is the anchor metric — every rate below is delivered-based, not attempted-based,
  // so a spike in failed/attempted sends no longer silently deflates the rates.
  const statCards = [
    { l:'Delivered',    v: t.delivered  || 0, color:'#057a55',      sub:'actually reached inbox' },
    { l:'Opens (Human)',v: t.opens      || 0, color:'#1a56db',      sub:`${r.open_rate||0}% of delivered` },
    { l:'Bot Opens',    v: t.bot_opens  || 0, color:'#6b7280',      sub:'excluded from rate' },
    { l:'Clicks',       v: t.clicks     || 0, color:'#7e3af2',      sub:`${r.click_rate||0}% of delivered` },
    { l:'Replies',      v: t.replies    || 0, color:'#8e4b10',      sub:`${r.reply_rate||0}% of delivered` },
    { l:'Failed',       v: t.failed     || 0, color:(t.failed||0)>0?'#c2410c':'var(--text)', danger:(t.failed||0)>0 },
  ];

  const campaignRows = (stats?.campaigns || []).slice().sort((a, b) => (b.delivered||0) - (a.delivered||0));
    ...
    <div>
      <div className="topbar">
        <div>
          ...
          <div className="topbar-sub">Delivered-based performance — see Analytics Logs for the full event-by-event view</div>
        </div>
        <div style={{ display:'flex', gap:8, flexWrap:'wrap', alignItems:'center' }}>
          ...
          <div style={{ display:'flex', border:'1px solid var(--border)', borderRadius:8, overflow:'hidden' }}>
            {[['today','Today'],['week','7d'],['month','30d'],['all','All']].map(([v,l]) => (
              <button key={v} onClick={() => setDate(v)} className="btn"
      ...
      <div className="page fade-in">
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(6,1fr)', gap:10, marginBottom:20 }}>
          {statCards.map(s => (
            <div key={s.l} className="stat-card" style={{ borderTop: s.danger ? '3px solid var(--danger)' : '3px solid transparent' }}>
              <div className="stat-label">{s.l}</div>
              <div className="stat-value" style={{ color: s.color, fontSize:22 }}>{statsLoading ? '…' : s.v}</div>
        ...
        {(t.bot_opens||0) > 0 && (
          <div style={{ marginBottom:16, padding:'10px 16px', background:'#fefce8', border:'1px solid #fde047', borderRadius:8, fontSize:13, color:'#713f12' }}>
            🤖 <strong>{t.bot_opens} bot opens detected</strong> — these are email security scanners, not real people, and are excluded from the open rate.
          </div>
        )}

        {/* ── Daily trend chart ── */}
        <div className="card" style={{ marginBottom:16 }}>
          <div style={{ fontWeight:600, fontSize:14, marginBottom:12 }}>Daily Trend</div>
          {statsLoading ? (
            <div style={{ padding:40, textAlign:'center', color:'var(--text-muted)' }}>Loading…</div>
          ) : stats?.daily?.length > 0 ? (
            <DailyTrendChart daily={stats.daily} />
          ) : (
            <div style={{ padding:40, textAlign:'center', color:'var(--text-muted)' }}>No data for this range</div>
          )}
        </div>
        ...
        <div className="card" style={{ padding:0, overflow:'hidden' }}>
          <div style={{ padding:'14px 20px', borderBottom:'1px solid var(--border)' }}>
            <div style={{ fontWeight:600, fontSize:14 }}>Campaign Breakdown</div>
            <div style={{ fontSize:12, color:'var(--text-muted)', marginTop:2 }}>Delivered-based rates per campaign</div>
          </div>
          {statsLoading ? (
            <div style={{ padding:40, textAlign:'center', color:'var(--text-muted)' }}>Loading…</div>
          ) : campaignRows.length === 0 ? (
            <div style={{ padding:40, textAlign:'center', color:'var(--text-muted)' }}>No campaign activity for this range</div>
          ) : (
            <div className="table-wrap">
              <table>
                <thead>
                  <tr>
                    <th>Campaign</th><th>Delivered</th><th>Opens</th><th>Clicks</th><th>Replies</th>
                    <th>Failed</th><th>Open %</th><th>Reply %</th>
                  </tr>
                </thead>
                <tbody>
                  {campaignRows.map(c => (
                    <tr key={c.campaign_id}>
                      ...
                      <td>{c.delivered||0}</td>
                      <td style={{ color:(c.opens||0)>0?'#057a55':undefined }}>{c.opens||0}</td>
                      <td>{c.clicks||0}</td>
                      <td>{c.replies||0}</td>
                      <td style={{ color:(c.failed||0)>0?'var(--warning)':undefined }}>{c.failed||0}</td>
                      <td>{c.delivered>0 ? ((c.opens/c.delivered)*100).toFixed(1)+'%' : '—'}</td>
                      <td>{c.delivered>0 ? ((c.replies/c.delivered)*100).toFixed(1)+'%' : '—'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        {/* ── Inbox breakdown — collapsed below ── */}
        {stats?.inboxes?.length > 0 && (
          <details style={{ marginTop:16 }}>
            <summary style={{ cursor:'pointer', padding:'12px 0', fontWeight:600, fontSize:13, color:'var(--text-secondary)', userSelect:'none' }}>
              Inbox Performance
            </summary>
                  ...
                  <tbody>
                    {stats.inboxes.map(i => (
                      <tr key={i.inbox}>
                        ...
                        <td>{i.sends}</td>
                        <td style={{ color:i.opens>0?'#057a55':undefined }}>{i.opens}</td>
                        <td>{i.replies}</td>
