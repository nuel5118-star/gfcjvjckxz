...
const VARS = [
  ...
  { key: 'watch_link', label: 'Watch Video Link', default_fallback: '' },
  { key: 'gif_url', label: 'Video GIF Preview URL', default_fallback: '' },
];

// One-click insert of the Loom-style "GIF that looks like a paused video,
// click it and it takes you to the real page" embed — the exact pattern the
// personalized landing page is built for. Wraps {{gif_url}} in a link to
// {{watch_link}}, both of which resolve per-contact automatically.
  ...
  return (
    <div style={{ marginBottom:10 }}>
      <div style={{ display:'flex', gap:6, flexWrap:'wrap', alignItems:'center', marginBottom:6 }}>
        ...
        <span
          className="var-pill"
          onClick={() => onInsert(VIDEO_GIF_SNIPPET)}
          title="Inserts a clickable GIF thumbnail (Loom-style) linking to this contact's personalized video page"
        >
...
// even after reopening the campaign later.
function findSourceInfo(sequences, sourceStepId) {
  if (!sourceStepId) return null;
  for (const seq of sequences || []) {
    const found = (seq.sequence_steps || []).find(ss => ss.id === sourceStepId);
    if (found) return { sequence: seq, sequenceStep: found };
  }
  return null;
}

function SourceBadge({ step, sequences }) {
  const info = findSourceInfo(sequences, step.source_sequence_step_id);
  if (!info) return null;
  const edited = step.subject !== info.sequenceStep.subject || step.body !== info.sequenceStep.body;
  return (
    <span style={{
      display: 'inline-flex', alignItems: 'center', gap: 5, fontSize: 11, fontWeight: 700,
      background: edited ? '#fff4e0' : 'var(--accent-light)', color: edited ? '#a15c00' : 'var(--accent)',
      padding: '3px 9px', borderRadius: 20, whiteSpace: 'nowrap',
    }}>
      📚 From: {info.sequence.name} · Email {info.sequenceStep.step_number}{edited ? ' (edited)' : ''}
    </span>
  );
}

// Persistent sidebar — stays open the whole time you're building, so picking
// an email never makes the library disappear. "Insert" always targets
// whichever step card was last clicked/focused (activeStepIndex).
function SequenceLibraryPanel({ sequences, activeStepIndex, totalSteps, onInsert }) {
  ...
  return (
    <div className="card seq-library-panel">
      ...
      <div style={{ fontSize: 12, fontWeight: 600, marginBottom: 10, background: 'var(--accent-light)', color: 'var(--accent)', padding: '6px 10px', borderRadius: 8 }}>
        Inserting into: Email {activeStepIndex + 1} of {totalSteps}
      </div>
      {sequences.map(seq => (
        <div key={seq.id} style={{ border: '1px solid var(--border)', borderRadius: 10, overflow: 'hidden', marginBottom: 12 }}>
          <div style={{ background: 'var(--bg-muted)', padding: '7px 10px', fontSize: 12, fontWeight: 700 }}>{seq.name}</div>
          {(seq.sequence_steps || []).length === 0 && (
            <div style={{ padding: '8px 10px', fontSize: 11, color: 'var(--text-muted)' }}>No emails in this sequence yet</div>
          )}
          {(seq.sequence_steps || []).map(ss => (
            <div key={ss.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '7px 10px', fontSize: 12, borderTop: '1px solid var(--border)' }}>
              <span style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: 150 }} title={ss.subject}>
                Email {ss.step_number} — {ss.subject || '(no subject)'}
              </span>
              <button type="button" onClick={() => onInsert(ss)} className="btn btn-secondary btn-sm" style={{ flexShrink: 0 }}>Insert →</button>
            </div>
          ))}
        </div>
      ))}
...
function StepCard({ step, index, total, onChange, onRemove, onMoveUp, onMoveDown, campaignId, sequences, links, isActive, onFocusStep }) {
  ...
  return (
    <div
      ...
      style={{
        borderColor: isActive ? 'var(--accent)' : showPreview ? 'var(--accent)' : undefined,
      ...
      <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:14, flexWrap:'wrap' }}>
        ...
        <div style={{ flex:1, minWidth:140 }}>
          <div style={{ fontWeight:600, fontSize:13, display:'flex', alignItems:'center', gap:8, flexWrap:'wrap' }}>
            Email {index + 1}
            <SourceBadge step={step} sequences={sequences} />
        ...
        <div style={{ display:'flex', alignItems:'center', gap:4 }}>
          ...
          <button type="button" onClick={e => { e.stopPropagation(); onMoveUp(index); }} disabled={index === 0} className="action-btn" title="Move up">▲</button>
          <button type="button" onClick={e => { e.stopPropagation(); onMoveDown(index); }} disabled={index === total - 1} className="action-btn" title="Move down">▼</button>
          <button type="button" onClick={e => { e.stopPropagation(); if (!showPreview) loadPreview(); setShowPreview(!showPreview); }} className="action-btn" style={{ color: showPreview ? 'var(--info)' : undefined }} title="Preview">
            👁
          </button>
          {total > 1 && <button type="button" onClick={e => { e.stopPropagation(); onRemove(index); }} className="action-btn danger">✕</button>}
        </div>
      </div>
...
export default function CampaignBuilder() {
  ...
  useEffect(() => {
    ...
    api.getCampaign(id).then(c => {
      ...
      const sorted = (c.campaign_steps || []).filter(s => (s.track || 'main') === 'main').sort((a, b) => a.step_number - b.step_number);
    ...
    <div>
      ...
      <div className="page fade-in campaign-builder-page" style={{ maxWidth:780 }}>
        ...
        <div style={{ marginBottom:16, display:'flex', gap:20, alignItems:'flex-start', flexWrap:'wrap' }}>
          ...
          {/* Persistent sequence library — stays open, never collapses on you */}
          <div style={{ flex:'0 1 280px', minWidth:240 }}>
            <SequenceLibraryPanel
              sequences={sequences}
              activeStepIndex={Math.min(activeStepIndex, steps.length - 1)}
              totalSteps={steps.length}
              onInsert={ss => setSteps(s => s.map((x, idx) => idx === activeStepIndex ? {
                ...x,
                subject: ss.subject,
                body: ss.body,
                delay_days: ss.delay_days || x.delay_days,
        ...
        {/* Intent Tracks — attaching one is the only thing that happens here;
            the track's own content is built on the Intent Tracks page,
            same as how Sequences work. Only available once the campaign
            actually exists, since an attachment needs a real campaign_id. */}
        {isEdit ? (
          <AttachedTracksSection campaignId={id} links={links} />
        ) : (
          <div className="card" style={{ marginBottom:16, fontSize:12, color:'var(--text-muted)' }}>
            🎯 Intent Tracks (switch a contact onto a different, separately-built sequence when they show interest) become available once you save this campaign for the first time.
          </div>
        )}
...
// Trigger types an attached Intent Track can react to. 'link_click' needs a
// specific saved link selected (any link in the Links Library — including
// video links, booking pages, anything); the others are reported by outside
// systems (the calculator's own webhook, a future reply-detection hook)
// calling POST /api/branch/trigger directly.
const TRIGGER_TYPES = [
...
function AttachedTracksSection({ campaignId, links }) {
  ...
  const load = () => {
    setLoading(true);
    Promise.all([
      api.getAttachedTracks(campaignId).catch(() => []),
      api.getIntentTracks().catch(() => []),
    ]).then(([a, l]) => { setAttached(a || []); setLibrary(l || []); }).finally(() => setLoading(false));
  };
  useEffect(() => { load(); }, [campaignId]);

  const reset = () => { setAdding(false); setTrackId(''); setTriggerType('link_click'); setLinkSlug(''); setErr(''); };

  const attach = async () => {
    if (!trackId) { setErr('Pick which Intent Track to attach'); return; }
    if (triggerType === 'link_click' && !linkSlug) { setErr('Pick which link this should react to'); return; }
    setSaving(true); setErr('');
    try {
      const selectedLink = links.find(l => l.slug === linkSlug);
      await api.attachTrack(campaignId, {
        intent_track_id: trackId,
  ...
  const detach = async (branchId) => {
    if (!confirm('Detach this track? Its emails will be removed from this campaign (contacts who already switched onto it would be blocked — you\'ll get a warning if that\'s the case).')) return;
    try {
      await api.detachTrack(campaignId, branchId);
      load();
  ...
  return (
    <div className="card" style={{ marginBottom:16 }}>
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:10, flexWrap:'wrap', gap:8 }}>
        <div style={{ fontWeight:700, fontSize:15 }}>
          ...
          <span style={{ fontWeight:400, fontSize:12, color:'var(--text-muted)', marginLeft:8 }}>
            switch a contact onto a separately-built sequence when they show interest
          </span>
        </div>
        <div style={{ display:'flex', gap:8 }}>
          <Link to="/intent-tracks" className="btn btn-secondary btn-sm">Manage Tracks Library</Link>
          {!adding && <button type="button" onClick={() => setAdding(true)} className="btn btn-secondary btn-sm">+ Attach Track</button>}
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div style={{ fontSize:12, color:'var(--text-muted)' }}>
          Nothing attached yet — everyone just gets the main sequence above. Build a track on the Intent Tracks page, then attach it here with a trigger (e.g. "when they submit the calculator").
        </div>
      ) : (
        <div style={{ display:'flex', flexDirection:'column', gap:8, marginBottom: adding ? 12 : 0 }}>
          {attached.map(b => (
            <div key={b.id} style={{ display:'flex', alignItems:'center', gap:10, padding:'8px 10px', background:'var(--bg-subtle)', borderRadius:8, fontSize:12 }}>
              <div style={{ flex:1 }}>
                <strong style={{ color:'var(--accent)' }}>{b.intent_tracks?.name || b.name}</strong>
                {' '}— switches on: <strong>{TRIGGER_TYPES.find(t => t.value === b.trigger_type)?.label || b.trigger_type}</strong>
                {b.trigger_meta?.url && <> (<span style={{ fontFamily:'monospace' }}>{b.trigger_meta.url}</span>)</>}
              </div>
              <button type="button" onClick={() => detach(b.id)} className="action-btn danger" title="Detach">✕</button>
            </div>
          ))}
        </div>
      )}
      ...
      {adding && (
        <div style={{ marginTop:12, padding:14, background:'var(--bg)', border:'2px solid var(--accent)', borderRadius:10 }}>
          ...
          {library.length === 0 ? (
            <div style={{ fontSize:12, color:'var(--text-muted)' }}>
              No Intent Tracks built yet — <Link to="/intent-tracks/new">create one</Link> first, then come back here to attach it.
            </div>
          ) : (
            <>
              <div className="form-group">
                <label className="label">Which track</label>
                <select className="input" value={trackId} onChange={e => setTrackId(e.target.value)}>
                  <option value="">— Select an Intent Track —</option>
                  {library.map(t => <option key={t.id} value={t.id}>{t.name} ({(t.intent_track_steps || []).length} emails)</option>)}
                </select>
              </div>
              <div className="form-group">
              ...
              <div style={{ display:'flex', gap:8, justifyContent:'flex-end' }}>
                ...
                <button type="button" onClick={attach} disabled={saving} className="btn btn-primary btn-sm">{saving ? 'Attaching...' : 'Attach Track'}</button>
              </div>
            </>
          )}
        </div>
