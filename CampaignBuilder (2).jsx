...
function StepCard({ step, index, total, onChange, onRemove, onMoveUp, onMoveDown, campaignId, sequences, isActive, onFocusStep }) {
  ...
  useEffect(() => {
    if ((step.send_hour_start != null || step.send_hour_end != null) && !showPerStepTime) setShowPerStepTime(true);
...
export default function CampaignBuilder() {
  ...
  return (
    <div>
      ...
      <div className="page fade-in campaign-builder-page" style={{ maxWidth:1200 }}>
        ...
        <div style={{ marginBottom:16 }}>
            ...
            {steps.map((s, i) => (
              <div key={i} style={{ display:'flex', alignItems:'center', gap:6, opacity: s.enabled === false ? 0.45 : 1 }}>
                <div style={{ padding:'6px 12px', borderRadius:8, border:`2px solid ${s.enabled === false ? 'var(--border)' : s.subject ? 'var(--accent)' : 'var(--border)'}`, background: s.enabled === false ? 'var(--bg)' : s.subject ? 'var(--accent-light)' : 'var(--bg)', fontSize:12, fontWeight:600, color: s.enabled === false ? 'var(--text-muted)' : s.subject ? 'var(--accent)' : 'var(--text-muted)', textAlign:'center' }}>
                  ...
                  {(s.send_hour_start != null || s.send_hour_end != null) && <div style={{ fontSize:9, color:'var(--info)' }}>{s.send_hour_start ?? '?'}–{s.send_hour_end ?? '?'}h</div>}
                </div>
          ...
          {/* Persistent sequence library — floats fixed by the side on wide screens (see
              .seq-library-panel in index.css), stacks below the steps on narrow ones. It
              never disappears while you're editing, no matter where you scroll to. */}
