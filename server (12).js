...
// Restricts which inboxes can send to which niches -- prevents irrigation/lawn
// and tree_removal leads from ever mixing through the same inbox. Backed by
// the inboxes.allowed_niches column (null/empty = no restriction, eligible
// for every niche) so this is editable via the /api/inboxes endpoints below
// instead of being hardcoded here.

// Returns the pool of inboxes allowed to send to a given niche. Inboxes with
// allowed_niches=null (or empty array) are treated as unrestricted and are
// always eligible, alongside any inbox that explicitly lists this niche.
function inboxesForNiche(allInboxes,niche){
  const pool=allInboxes.filter(i=>{
    const restriction=i.allowed_niches;
    if(!restriction||!restriction.length)return true;// unrestricted inbox
    return niche&&restriction.includes(niche);
  });
  // If restrictions leave nothing eligible (e.g. a niche not assigned to any
  // inbox, and no unrestricted inbox exists), fail safe to the full list
  // rather than silently blocking sends for that niche.
  return pool.length?pool:allInboxes;
}
...
app.post('/api/campaigns/:id/launch',async(req,res)=>{
  ...
  // Round-robin index tracked PER NICHE POOL, not globally -- otherwise a
  // niche restricted to one inbox would still "consume" a turn every time
  // any contact (of any niche) was processed, skewing distribution.
  const inboxIdxByNiche={};
  for(let i=0;i<contacts.length;i++){
    if(countOnCurrentDay>=maxPerDay){dayOffset++;countOnCurrentDay=0;}
    const contact=contacts[i];
    const nichePool=inboxesForNiche(inboxes,contact.niche);
    const idx=inboxIdxByNiche[contact.niche]||0;
    const inbox=nichePool[idx%nichePool.length];
    inboxIdxByNiche[contact.niche]=idx+1;
...
app.post('/api/inboxes',async(req,res)=>{const{email,label,daily_cap,allowed_niches}=req.body;if(!email||!isValidEmail(email))return res.status(400).json({error:'Invalid email'});const{data,error}=await supabase.from('inboxes').upsert({email:normalizeEmail(email),label,daily_cap:daily_cap||100,active:true,allowed_niches:allowed_niches&&allowed_niches.length?allowed_niches:null},{onConflict:'email'}).select().single();if(error)return res.status(500).json({error:error.message});res.json(data);});
app.put('/api/inboxes/:id',async(req,res)=>{const{label,active,daily_cap,allowed_niches}=req.body;const update={label,active,daily_cap};if(allowed_niches!==undefined)update.allowed_niches=allowed_niches&&allowed_niches.length?allowed_niches:null;const{data,error}=await supabase.from('inboxes').update(update).eq('id',req.params.id).select().single();if(error)return res.status(500).json({error:error.message});res.json(data);});
// Distinct list of niches currently in use, for populating a picker in the UI
// -- so the niche-assignment control doesn't need its own hardcoded list.
app.get('/api/niches',async(req,res)=>{const{data,error}=await supabase.from('contacts').select('niche').not('niche','is',null);if(error)return res.status(500).json({error:error.message});const niches=[...new Set((data||[]).map(r=>r.niche).filter(Boolean))].sort();res.json(niches);});
...
async function runForceSendTest(campaignId){
  ...
  for(const contact of contacts){
    ...
    if(!inbox){
      const nichePool=inboxesForNiche(inboxes,contact.niche);
      if(!nichePool.length){result.skipped++;result.skip_reasons.no_inbox_available=(result.skip_reasons.no_inbox_available||0)+1;continue;}
      inbox=nichePool[inboxIdx%nichePool.length].email;inboxIdx++;
    }
