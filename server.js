...
async function runForceSendTest(campaignId){
  ...
  const contacts=await fetchAll(()=>supabase.from('contacts')
    .select('id,email,first_name,last_name,company,custom_fields,current_step,assigned_inbox,campaign_id,track,page_slug,niche')
    .eq('status','active')
  ...
  for(const contact of contacts){
    ...
    const customFields={
      ...
      gif_url: getGifUrlForNiche(settings, contact.niche)
    };
    ...
    while(true){
      ...
      for(const campaign of activeCampaigns){
        ...
        const{data:batch,error:queryError}=await supabase.from('contacts')
          .select('id,email,first_name,last_name,company,custom_fields,current_step,next_send_at,assigned_inbox,campaign_id,track,page_slug,niche')
          .eq('status','active')
          .eq('campaign_id',campaign.id)
