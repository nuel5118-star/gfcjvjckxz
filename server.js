...
// ── LANDING PAGE ROUTE (legacy / fallback) ────────────────────────
// Once LANDING_PAGE_DOMAIN is set, every /w/:slug hit on Railway 302s
// straight to the custom domain — so any link already built as
// APP_URL + /w/<slug> anywhere in the app (or already sent in a past
// email) starts pointing at the new custom-domain page automatically,
// with no other code changes required.
// Until LANDING_PAGE_DOMAIN is set, this falls back to rendering the
// page directly from Railway exactly as before, so nothing breaks
// mid-transition.
